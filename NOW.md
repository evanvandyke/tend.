...
- **🚨 CRITICAL: Projects don't surface on Now page properly.** A project with a due date (e.g., "Plant trees and bushes" due May 23) shows on the Tasks page under Project Tasks with the correct date, but does NOT appear on the Now page's "This Week" or "Coming Up" sections. The Now page is the primary interface — if tasks/projects with due dates don't surface there, users can't trust the app and won't use it. The `getNowFeed` query likely filters projects into their own "Open Projects" section but doesn't bucket them by due date like regular tasks. Projects with due dates need to appear in "This Week" / "Coming Up" based on their `dueAt`, not just in a separate projects section. Also: project detail view is bare — no date shown, no description, just title + edit/complete. Needs to show due date, subtasks, and notes.
- **Tasks page sorting:** Tasks are currently in random order — should be sorted by due date ascending (soonest first) within each section. Also add a sort toggle: "By Category" (current grouped sections: Single Tasks, Project Tasks, Routine Tasks with date-sorted items) vs "By Date" (flat chronological list, all tasks mixed, with inline colored dot + kind label to show type). Default to By Date since that's how most people think.
- **Header layout rebalance:** Currently everything is crammed on the right side. Move date + season (day of week, month/day, 🌿 Spring) to the LEFT side underneath "Tend." wordmark. Keep weather + lunar on the RIGHT side. This distributes visual weight evenly and makes the header less lopsided on mobile.
- **Weather doesn't update without hard refresh:** Weather is server-rendered in the app layout, which only runs once on initial load. Client-side navigation between tabs reuses the layout so weather goes stale. Need to refactor weather into a client-side fetch in TopBar with periodic polling (e.g., every 5–10 minutes via `useEffect` + interval), hitting `/api/weather` directly. This also fixes the issue where new users don't see weather until they force-refresh after onboarding sets their ZIP.
